...
namespace ffarray {
  ...
  template <class T>
    ...
    public:
      ...
      Array1D<double> makeKernel(float, float);
      int sizeKernel, x0;
      Array1D smooth1D(float, float);
...
{ 
#ifdef TNT_BOUNDS_CHECK
  if ((i<0)||(i>n_)) {throw std::invalid_argument("wrong!!");}
  //assert(i>= 0);
  //assert(i < n_);
#endif
  return data_[i]; 
...
void Array1D<T>::copy_(T* p, const T* q, int len) const
{
  ...
  */
template <class T>
Array1D<double> Array1D<T>::makeKernel(float old_x, 
					 float new_x)
  {
    const float bcorr=2.35; // FWHM = 2.35 sigma
    const float bcorr2=1.067; // for Gaussian convolution
    
    float sig=(new_x/old_x/bcorr/bcorr2);
    
    sizeKernel=(int)(5.*sig+0.5);
    ...
    for (int i=0;i<sizeKernel;i++)
      kernel[i]=1/2./PI/sig/sig*exp(-(i-x0)*(i-x0)/2./sig/sig);
    
    return kernel;
  }
  
  template <class T>
  Array1D<T> Array1D<T>::smooth1D(float old_x, 
				  float new_x)
  {
    Array1D<double> kernel=makeKernel(old_x, new_x);
    
    Array1D<T> smooA(n_, 0.);
    
    for (int i=sizeKernel/2;i<n_-sizeKernel/2;i++)
      for (int ki=0;ki<sizeKernel;ki++){
	if (v_[i-x0+ki] > 0)
	  smooA[i]+=kernel[ki]*v_[i-x0+ki];
      }
    return smooA;
  }
